...
export function Toast({ message, type, duration = 3000, onClose }: ToastProps) {
  ...
  const colors = {
    ...
    info: { bg: theme.colors.primary, text: '#FFFFFF' }
  };
  ...
  return (
    ...
    >
      <span style={{ color: colors[type].text }} className="shrink-0">{icons[type]}</span>
      <span className="text-sm font-bold flex-1" style={{ color: colors[type].text }}>{message}</span>
      <button 
        onClick={onClose} 
        className="ml-2 p-1 rounded-full hover:bg-black/10 transition-colors shrink-0"
      >
        <X className="w-4 h-4" style={{ color: colors[type].text }} />
      </button>
    </motion.div>
  );
}
